...
point_admin_logistic = {'AE': [], 'SM': [], 'CD': [], 'GAP': [], 'GI+LDI': [], 'SG+LDE': [], 'CFO': [], 'CEO': []}
# Var of Sheet Three
other_income = []
other_egress = []
buy_egress = []
...
def search_all_id_of_products():
    # Identificar todos los ID en todos los archivos y realizar una lista
    try:
        ids = {}
        for file in fileList:
            data_sheet_8 = pd.read_excel(file, sheet_name=8)  # Seller Summary
            products_id = list(data_sheet_8['Resumen'][1:501])

            for temp_id in products_id:
                if temp_id not in ids and str(temp_id) != 'nan':
                    ids[temp_id] = len(ids)
        return ids
    except Exception as e:
        print(repr(e))
        messagebox.showwarning('Consolidate',
                               'Verifique todos los archivos excel\n Puede que tenga alguno no estandar.\n\n Error 001')


def swap(list_to_swap, correct_index, failed_index):
    list_to_swap[correct_index], list_to_swap[failed_index] = list_to_swap[failed_index], list_to_swap[correct_index]


ids_unique = search_all_id_of_products()
...
for file in fileList:
    try:
        ...
        try:
            ...
            dayDict = [data_sheet_8[data_sheet_8.columns[29]][1:501]]  # Cant Venta (AD)
            dayDict += [data_sheet_8[data_sheet_8.columns[38]][1:501]]  # Inv.IR (AM)
            dayDict += [data_sheet_8[data_sheet_8.columns[39]][1:501]]  # In (AN)
            dayDict += [data_sheet_8[data_sheet_8.columns[42]][1:501]]  # Out (AO)
            dayDict += [data_sheet_8[data_sheet_8.columns[45]][1:501]]  # Inv.FT (AT)
            dayDict += [data_sheet_8[data_sheet_8.columns[46]][1:501]]  # Inv.FR (AU)
            dayDict += [(data_sheet_8[data_sheet_8.columns[46]][1:501] - data_sheet_8[data_sheet_8.columns[45]][
                                                                         1:501])]  # Desfasaje
            dayDict += [data_sheet_8[data_sheet_8.columns[54]][1:501]]  # Cost.V (BC)
            dayDict += [data_sheet_8[data_sheet_8.columns[55]][1:501]]  # Ing.V (BD)
            dayDict += [data_sheet_8[data_sheet_8.columns[56]][1:501]]  # Result (BE)
            dayDict += [data_sheet_8[data_sheet_8.columns[58]][1:501]]  # Merc.V (BG)
            dayDict += [(data_sheet_8[data_sheet_8.columns[59]][1:501] * -1)]  # Desc.Inv (BH)
            dayDict += [data_sheet_8[data_sheet_8.columns[31]][1:501]]  # Cant.C(AF)
            dayDict += [data_sheet_8[data_sheet_8.columns[32]][1:501]]  # Egr.C (AG)
            print('here 2.14')

            # Account Values
            id_temp = list(data_sheet_1[data_sheet_1.columns[23]][35:536])  # id
            name_temp = list(data_sheet_1[data_sheet_1.columns[22]][35:536])  # nombres
            ir_temp = list(data_sheet_1[data_sheet_1.columns[24]][35:536])  # ir
            ft_temp = list(data_sheet_9[data_sheet_9.columns[12]][28:228])  # ft
            fr_temp = list(data_sheet_9[data_sheet_9.columns[11]][28:228])  # fr
            # Total of Account Group
            if type(total_ir_group) is int:
                total_ir_group = data_sheet_1[data_sheet_1.columns[23]][23:33]
                total_ft_group = data_sheet_9[data_sheet_9.columns[11]][16:26]
                total_fr_group = data_sheet_9[data_sheet_9.columns[10]][16:26]
            else:
                total_ir_group += data_sheet_1[data_sheet_1.columns[23]][23:33]
                total_ft_group += data_sheet_9[data_sheet_9.columns[11]][16:26]
                total_fr_group += data_sheet_9[data_sheet_9.columns[10]][16:26]
            ...
            types_account[sellManagersNames[count]] = {'type': list(data_sheet_1[data_sheet_1.columns[22]][23:33]),
                                                       ...
                                                       'ft': list(data_sheet_9[data_sheet_9.columns[11]][16:26]),
                                                       'fr': list(data_sheet_9[data_sheet_9.columns[10]][16:26])}
            ...
            real_cash.append(float(data_sheet_9[data_sheet_9.columns[6]][19]))
            ...
            point_admin_logistic['GAP'].append(float(data_sheet_9[data_sheet_9.columns[9]][4]))
            point_admin_logistic['GI+LDI'].append(float(data_sheet_9[data_sheet_9.columns[9]][5]))
            point_admin_logistic['SG+LDE'].append(float(data_sheet_9[data_sheet_9.columns[9]][6]))
            point_admin_logistic['CFO'].append(float(data_sheet_9[data_sheet_9.columns[9]][7]))
            point_admin_logistic['CEO'].append(float(data_sheet_9[data_sheet_9.columns[9]][8]))
        ...
        if correctFiles:
            ...
            # Verificar la columna ID en la hoja 8 que correspondan todos los archivos para que no se desfasen los datos
            print('Begin -', sellManagersNames[count])
            for key in ids_unique:
                if key not in products_id:
                    size = len(products_id) - 1
                    while str(products_id[size]) != 'nan':
                        size -= 1

                    for list_to_swap in dayDict:
                        list_to_swap[ids_unique[key]], list_to_swap[size] = 0, list_to_swap[ids_unique[key]]

                    products_id[ids_unique[key]], products_id[size] = key, products_id[ids_unique[key]]

                elif ids_unique[key] != products_id.index(key):
                    # Swapping
                    failed_index = products_id.index(key)
                    correct_index = ids_unique[key]

                    # temp = products_id.copy()
                    for list_to_swap in dayDict:
                        swap(list_to_swap, correct_index, failed_index)

                    # swap(dayDict[13], correct_index, failed_index)
                    swap(products_id, correct_index, failed_index)
...
if correctFiles and len(sellManagersNames) > 0:
    ...
    for sm in sellManagersNames:
        ...
        column_cost = pd.DataFrame(
            {
                only_one_decimal(totalManager[sm]['Cost.V'], symbol=True): ['Cost.V.'] + [None if value == 0 else value
                                                                                          for value in
                                                                                          list(managerDict[sm][7])]
            }
        )

        column_sell = pd.DataFrame(
            {
                only_one_decimal(totalManager[sm]['Ing.V'], symbol=True): ['Ing.V'] + [None if value == 0 else value for
                                                                                       value in
                                                                                       list(managerDict[sm][8])]
            }
        )
        column_result = pd.DataFrame(
            {
                only_one_decimal(totalManager[sm]['Result'], symbol=True): ['Result'] + [None if value == 0 else value
                                                                                         for value in
                                                                                         list(managerDict[sm][9])]
            }
        )
        column_merc = pd.DataFrame(
            {
                only_one_decimal(totalManager[sm]['Merc.V'], symbol=True): ['Merc.V'] + [None if value == 0 else value
                                                                                         for value in
                                                                                         list(managerDict[sm][10])]
            }
        )

        column_desc_inv = pd.DataFrame(
            {
                only_one_decimal(totalManager[sm]['Desc.DI'], symbol=True): ['Desc.DI'] + [None if value == 0 else value
                                                                                           for value in
                                                                                           list(managerDict[sm][11])]
            }
        )
        ...
        column_buy = pd.DataFrame(
            {
                only_one_decimal(totalManager[sm]['Egr.C'], symbol=True): ['Egr.C'] + [None if value == 0 else value for
                                                                                       value in
                                                                                       list(managerDict[sm][13])]
            }
        )
    ...
    column_CantV_sheet_2 = pd.DataFrame(
        {'': ['Cant.V'] + [None if value == 0 else value for value in list(total_amount)]})
    column_CantC_sheet_2 = pd.DataFrame(
        {'': ['Cant.C'] + [None if value == 0 else value for value in list(total_amount_buy)]})
    column_Inv_Sem_Ant_sheet_2 = pd.DataFrame(
        {'': ['Inv.IR'] + [None if value == 0 else value for value in list(total_inv_sem_ant)]})
    column_Inv_Teorico_sheet_2 = pd.DataFrame(
        {'': ['Inv.FT'] + [None if value == 0 else value for value in list(total_inv_teorico)]})
    column_Inv_Real_sheet_2 = pd.DataFrame(
        {'': ['Inv.FR'] + [None if value == 0 else value for value in list(total_inv_real)]})
    column_Desfasaje_sheet_2 = pd.DataFrame(
        {
            only_one_decimal(sum(total_desfasaje)): ['Desf.Inv'] + [None if value == 0 else value for value in
                                                                    list(total_desfasaje)]})
    column_CostV_sheet_2 = pd.DataFrame(
        {
            only_one_decimal(sum(total_cost_v)): ['Cost.V.'] + [None if value == 0 else value for value in
                                                                list(total_cost_v)]})
    column_Venta_sheet_2 = pd.DataFrame(
        {only_one_decimal(sum(total_venta)): ['Ing.V'] + [None if value == 0 else value for value in
                                                          list(total_venta)]})
    column_Compra_sheet_2 = pd.DataFrame(
        {only_one_decimal(sum(total_egress_buy)): ['Egr.C'] + [None if value == 0 else value for value in
                                                               list(total_egress_buy)]})
    column_Result_sheet_2 = pd.DataFrame(
        {only_one_decimal(sum(total_result)): ['Result'] + [None if value == 0 else value for value in
                                                            list(total_result)]})
    column_Merc_V_sheet_2 = pd.DataFrame(
        {only_one_decimal(sum(total_merc_v)): ['Merc.V.'] + [None if value == 0 else value for value in
                                                             list(total_merc_v)]})
    column_Desf_Inv_sheet_2 = pd.DataFrame(
        {only_one_decimal(sum(total_desc_inv)): ['Desc.DI'] + [None if value == 0 else value for value in
                                                               list(total_desc_inv)]})
    ...
    sheet_frame_4 = pd.DataFrame({
        ...
        'Punto de Venta': ['Vendedor'] + list(point_of_sale['Vendedor']),
        'Asistente': ['Asistente'] + list(point_of_sale['Asistente']),
        'Dueno': ['Dueño'] + list(point_of_sale['Dueno']),
        'Punto Administración y Logística': ['AE'] + list(point_admin_logistic['AE']),
        ' ': ['SM'] + list(point_admin_logistic['SM']),
        '  ': ['CD'] + list(point_admin_logistic['CD']),
        sum(point_admin_logistic['GAP'])+0.0001: ['GAP'] + list(point_admin_logistic['GAP']),
        sum(point_admin_logistic['GI+LDI'])+0.00001: ['GI+LDI'] + list(
            point_admin_logistic['GI+LDI']),
        sum(point_admin_logistic['SG+LDE'])+0.00003: ['SG+LDE'] + list(
            point_admin_logistic['SG+LDE']),
        sum(point_admin_logistic['CFO'])+0.00004: ['CFO'] + list(point_admin_logistic['CFO']),
        sum(point_admin_logistic['CEO'])+0.00005: ['CEO'] + list(point_admin_logistic['CEO'])
    })

    temp_sheet_4 = pd.DataFrame({
        'Total point_of_sale': sheet_frame_4['Punto de Venta'][1:] + sheet_frame_4['Asistente'][1:] + sheet_frame_4[
                                                                                                          'Dueno'][1:],
        'Total point_admin_logistic': sheet_frame_4['Punto Administración y Logística'][1:] +
                                      point_admin_logistic['GAP'] + point_admin_logistic['GI+LDI'] +
                                      point_admin_logistic['SG+LDE'] +
                                      point_admin_logistic['CFO'] + point_admin_logistic['CEO'],
    ...
    column_CostV_sheet_3 = pd.DataFrame(
        {only_one_decimal(sum(values_sm['Cost.V'])): ['Cost.V'] + [None if value == 0 else value for value in
                                                                   values_sm['Cost.V']]})
    column_Venta_sheet_3 = pd.DataFrame(
        {only_one_decimal(sum(values_sm['Ing.V'])): ['Ing.V'] + [None if value == 0 else value for value in
                                                                 values_sm['Ing.V']]})
    ...
    column_Result_sheet_3 = pd.DataFrame(
        {temp_result: ['Result'] + [None if value == 0 else value for value in values_sm['Result']]})
    column_MercV_sheet_3 = pd.DataFrame(
        {only_one_decimal(sum(values_sm['Merc.V'])): ['Merc.V'] + [None if value == 0 else value for value in
                                                                   values_sm['Merc.V']]})
    column_Desf_Inv_sheet_3 = pd.DataFrame(
        {temp_desfInv + '      ': ['Desc.DI'] + [None if value == 0 else value for value in values_sm['Desc.DI']]})
    column_Other_Ing_sheet_3 = pd.DataFrame(
        {only_one_decimal(sum(other_income)): ['Otros Ingr.'] + [None if value == 0 else value for value in
                                                                 other_income]})
    column_Other_Egr_sheet_3 = pd.DataFrame(
        {only_one_decimal(sum(other_egress)): ['Otros Egr'] + [None if value == 0 else value for value in
                                                               other_egress]})
    ...
    column_PointV_sheet_3 = pd.DataFrame({only_one_decimal(sum(temp_sheet_4['Total point_of_sale'])): [
                                                                                                          'Punto Venta'] + [
                                                                                                          None if value == 0 else value
                                                                                                          for value in
                                                                                                          temp_sheet_4[
                                                                                                              'Total point_of_sale']]})
    column_Admin_sheet_3 = pd.DataFrame({only_one_decimal(sum(temp_sheet_4['Total point_admin_logistic'])): [
                                                                                                                'Admin y Logistica'] + [
                                                                                                                None if value == 0 else value
                                                                                                                for
                                                                                                                value in
                                                                                                                temp_sheet_4[
                                                                                                                    'Total point_admin_logistic']]})
    ...
    column_K_Retenido_sheet_3 = pd.DataFrame(
        {only_one_decimal(sum([x if x > 0 else 0 for x in final_score] - dividends)): ['K-Retenido'] + list(
    ...
    worksheet_5.freeze_panes(2, 1)
    worksheet_6.freeze_panes(2, 1)
    worksheet_7.freeze_panes(2, 1)
    worksheet_8.freeze_panes(2, 1)
    ...
    for col_num, value in enumerate(sheet_frame_4.columns.values):
        if col_num == 1:
            ...
            worksheet_4.merge_range(0, 4, 0, 6, value, workbook.add_format(
                {
                    'valign': 'vcenter',
                    'align': 'center',
                    'bg_color': '#00C85A',
    ...
    for i in range(len(sellManagersNames)):
        ...
        count += 1
        differ_than(array_frame_1, 2, 501, count, count - 1, worksheet_1, workbook)  # Real Inventary
        worksheet_1.write(1, count, array_frame_1[0][count], workbook.add_format(
            {
                'valign': 'vcenter',
                'align': 'center',
        ...
        count += 1
        positive_and_negative_numbers_print(array_frame_1, 2, 501, count, worksheet_1, workbook)  # Inventory Mismatch
        worksheet_1.write(1, count, array_frame_1[0][count], workbook.add_format(
            {
                'valign': 'vcenter',
                'align': 'center',
        ...
        # negative_numbers_print(array_frame_1, 2, 501, count, worksheet_1, workbook)
        count += 1
    ...
    differ_than(array_frame_2, 2, 501, 5, 4, worksheet_2, workbook)
    positive_and_negative_numbers_print(array_frame_2, 2, 501, 6, worksheet_2, workbook)
    ...
    # positive_and_negative_numbers_print(array_frame_2, 2, 501, 5, worksheet_2, workbook)
    ...
    negative_numbers_print(array_frame_2, 2, 501, 12, worksheet_2, workbook)
    ...
    # negative_numbers_print(array_frame_2, 2, 501, 10, worksheet_2, workbook)
