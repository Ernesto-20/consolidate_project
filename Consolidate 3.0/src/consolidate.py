...
def consolidate(file_list, file_name, file_model, name_model, file_stock):
    ...
    for file in file_list:
        ...
        is_pivot = name == 'jor'
        worker = WorkerFile(name, ResumeOneManage.get_resume_one(ws_resumen_1, is_pivot=is_pivot),
        ...
        if is_pivot:
            date = ws_resumen_1.cell(row=2, column=1).value
            week_number = ws_resumen_1.cell(row=1, column=3).value
    ...
    for worker_file in worker_files_list:
        ...
        ResumeDivManage.set_resume_div(resume_2_ws, worker_file)
